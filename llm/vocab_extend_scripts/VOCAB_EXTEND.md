...
## 1.结果展示
词表扩充预训练后，经过sft效果对比（{<font color=Blue>数字一</font>}/{<font color=Red>数字二</font>}/{<font color=Green>数字三</font>}，分别对应{<font color=Blue>扩充前facebook/llama-7b</font>}/{<font color=Red>扩充后facebook/llama-7b</font>}/{<font color=Green>扩充前的Qwen/qwen-7b</font>}）：
结果表：
...
#训练结果参数和主干模型合并
python merge_lora_params.py  --use_vocab_extend true
#后续SFT同正常的SFT方案
```
**Note:**
为了实现TP(Tensor Parallel)分布式策略，需要使用model_param_vocab_resize.py先保存词表扩充后的模型参数，再进行训练
## 3.参数介绍
- `pretrain_files_dir`：
